...
if [[ "$2" =~ ^(cwm|cav|link|cxd|vgm|ftm|crv)$ ]]; then
    vault_asset=$2
else
    echo "Select which vault"
    echo -n "cwm = camWMATIC, cav = camAAVE, link = LMVT, cxd = cxDOGE, vgm = vGMVT, ftm = FTMVT, crv=CMVT: "
    read vault_asset
fi
...
declare -A functions=( ["cwm"]="camWMATIC" ["cav"]="camAAVE" ["link"]="LMVT" ["cxd"]="cxDMVT" ["vgm"]="vGMVT" ["ftm"]="FTMVT" ["crv"]="CMVT")
